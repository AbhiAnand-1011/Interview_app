...
function JoinInterviewModal({sendOffer,callBack,onClose}) {
    const handeParentCallback=useCallback(async ()=>{
        const id=document.getElementById("meeting-code").value;
        callBack(id);
        await sendOffer(id);
        onClose();
    })
  return (
    ...
    <div className='fixed inset-0 bg-black bg-opacity-50 backdrop-blur-lg flex justify-center items-center'>
      <div className='bg-[#1E1E1E] border border-gray-700 rounded-lg p-5 w-[60%] max-w-md shadow-lg relative mt-5 flex flex-col gap-2 text-white'>
            ...
            <input type="text" id="meeting-code"
            ...
            <div className='mt-3 flex justify-end space-x-2'>
                <button className="px-5 py-1 bg-gray-700 rounded-lg text-gray-300 hover:bg-gray-600" onClick={onClose}>Cancel</button>
               
                <button className="px-6 py-1 bg-green-800 rounded-lg text-white hover:bg-green-900" onClick={handeParentCallback}>Join Meeting</button>
